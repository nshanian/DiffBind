...
[Click here](https://htmlpreview.github.io/?https://github.com/nshanian/Documents/blob/main/DiffBind-atac.html) for the HTML version of this workflow with the output and the embedded plots.

The required inputs are generated by alignment and peak calling of raw ATAC-seq data in `.fastq` format. For workflows that would perform those analyses see the related [ATAC-seq](https://github.com/nshanian/ATAC-seq) repository. 

## Setup

The `diffbind_atac.R` script is written to run in an `R` environment as a part of SLURM submission on a high performance computing (HPC) cluster. It is NOT written to run locally using `R` or `RStudio` since the necessary `.bam` files called in the `sampleSheet` are typically too large. However, you may try running it the 'Terminal' portion of `RStudio` once you connect to a cluster that can run `R`. 
...
Save `diffbind_atac.R` script in your working directory, together with the modified `diffbind.sh` shell script and `sampleSheet` metafile listing covariates and experimental factors. Examples of both are provided in this repository. 
...
The data set used in this workflow comes from propionyl ATAC-seq in CRC cells. The untreated group is used as a control. The data identifies genomic regions marked by increased binding of transpose or chromtin accessiblity measured by ATAC-seq following treatment with sodium propionate (10 mM). 
